...
<p align="center">
  ...
  注册一次 CLI，以后用短命令在 Ghostty 或 iTerm2 里自动打开 2x2 工作区。
</p>
...
`tx add claude -cc` 会同时注册 `tx claude` 和 `tx cc`。执行任意一个都会打开你配置的终端，自动切成 2x2 四个 pane，进入你执行 `tx` 时所在的目录，并在四个 pane 里分别启动 `claude`。

## 终端

`tx` 支持 Ghostty 和 iTerm2。Ghostty 使用 1.3+ 的原生 AppleScript 分屏 API；iTerm2 使用 tmux 来稳定创建 2x2 布局。
...
- macOS
- Ghostty 1.3+ 或 iTerm2
- tmux，仅 iTerm2 后端需要
- Python 3

## 这个东西解决什么

有些 CLI 很适合四开：一个 pane 跑主任务，一个 pane 做实验，一个 pane 看日志，一个 pane 备用。`tx` 负责稳定创建四 pane 布局，终端只负责承载你的工作窗口。
